...
返回JSON，不要有多余文字：
{
  "entities": [{"name_cn": "实体中文名", "name_en": "EntityEnglish", "type": "Organization|Product|Material|Category|Document|Process|Facility|Concept", "description": "描述", "properties": {"属性名": "值"}, "property_schema": {"可量化字段名": {"type": "number|string|boolean", "unit": "可选单位"}}, "confidence": 0.9}],
  "relations": [{"source": "实体A的name_cn", "target": "实体B的name_cn", "type": "IS-A|PART-OF|INSTANCE-OF|supply|stores|processes|关联", "confidence": 0.85}],
  "logic_rules": [{"name_cn": "规则名", "name_en": "RuleName", "formula": "IF 条件 THEN 结论", "description": "规则描述", "confidence": 0.9, "linked_entities": ["实体name_cn"], "conditions": [{"field": "字段名", "op": ">|<|>=|<=|==|!=|in", "value": 阈值}]}],
  "actions": [{"name_cn": "动作名", "name_en": "ActionName", "execution_rule": "触发条件及执行逻辑", "description": "动作描述", "confidence": 0.9, "linked_entities": ["实体name_cn"], "linked_logic_names": ["逻辑规则name_cn"], "target_entity_type": "目标实体类型的name_en", "submission_criteria": [{"field": "字段名", "op": ">|<|>=|<=|==|!=|in", "value": 阈值}], "function_code": "def action_name(context: dict) -> dict:\\n    return {'status': 'ok'}"}]
}

【新增字段说明】
- property_schema: 描述该实体类型"有哪些可量化属性字段"（schema定义，不是具体数值）。
  只列出后续规则/动作可能做数值比较或状态判断的字段。
  例如 {"射程_km": {"type": "number", "unit": "km"}, "状态": {"type": "string"}}
  注意：每个字段必须是单一可比较值，不要输出混合多个值的字符串。
- conditions: 结构化条件数组，field 只能引用对应实体 property_schema 中已定义的字段名。
  op 只允许: >, <, >=, <=, ==, !=, in
- submission_criteria: 动作的提交条件，格式同 conditions。
- target_entity_type: 该动作作用于哪种实体类型，填 entities 中已出现的 name_en（英文名）。"""},
...
{
  "entities": [
    {"name_cn": "中文名", "name_en": "EnglishName", "type": "Supplier|Product|Material|Warehouse|Document|Category|Process", "description": "描述", "properties": {"属性1": "值", "属性2": "值"}, "property_schema": {"可量化字段名": {"type": "number|string|boolean", "unit": "可选单位"}}, "confidence": 0.95}
  ],
  "relations": [
    {"source": "实体A的name_cn", "target": "实体B的name_cn", "type": "supply|IS-A|PART-OF|INSTANCE-OF|stores|processes|关联", "confidence": 0.85}
  ],
  "logic_rules": [
    {"name_cn": "规则名", "name_en": "RuleName", "formula": "IF 条件 THEN 结论", "description": "描述", "confidence": 0.9, "linked_entities": ["实体name_cn_1", "实体name_cn_2"], "conditions": [{"field": "字段名", "op": ">|<|>=|<=|==|!=|in", "value": 阈值}]}
  ],
  "actions": [
    {"name_cn": "动作名", "name_en": "ActionName", "execution_rule": "触发条件及执行逻辑", "description": "描述", "confidence": 0.9, "linked_entities": ["实体name_cn_1"], "linked_logic_names": ["逻辑规则name_cn"], "target_entity_type": "目标实体类型的name_en", "submission_criteria": [{"field": "字段名", "op": ">|<|>=|<=|==|!=|in", "value": 阈值}], "function_code": "def action_name(context: dict) -> dict:\\n    val = context.get('key', 0)\\n    if val < context.get('threshold', 100):\\n        return {'status': 'triggered', 'message': '已触发'}\\n    return {'status': 'skipped', 'message': '条件未满足'}"}
  ]
}"""},
    ...
    {"name": "教育本体提取", "domain": "教育", "content": """从教育文档中提取课程、知识点、能力等概念及关系，返回JSON，不要有多余文字：
{
  ...
  "actions": []
    ...
    {"name": "Phase 2 本体提取（类型+实例双层）", "domain": "军事", "content": """你是军事领域本体工程专家。请从以下文档中提取 Palantir 风格双层本体。
...
{
  "object_types": [
    {"name_cn": "类型中文名", "name_en": "TypeEnglish", "description": "描述",
     "property_schema": {"字段名": {"type": "number|string|boolean", "unit": "可选单位"}},
     "interface_ids": []}
  ],
  "object_instances": [
    {"name_cn": "实例中文名", "name_en": "InstanceEnglish", "description": "描述",
     "object_type_id": "对应的object_types中某条记录的临时ID（如 ot-1）",
     "properties": {"字段名": "具体值"}}
  ],
  "interfaces": [
    {"name_cn": "接口中文名", "name_en": "InterfaceEnglish", "description": "描述",
     "shared_properties": [{"name": "字段名", "type": "number|string|boolean", "description": "说明"}]}
  ],
  "link_types": [
    {"name_cn": "关系中文名", "name_en": "RelationEnglish", "description": "描述",
     "source_object_type_temp_id": "源类型临时ID(可空=不限)", "target_object_type_temp_id": "目标类型临时ID(可空=不限)"}
  ],
  "links": [
    {"link_type_temp_id": "关系类型临时ID",
     "source_instance_temp_id": "源实例临时ID",
     "target_instance_temp_id": "目标实例临时ID"}
  ],
  "logic_rules": [
    {"name_cn": "规则名", "formula": "IF 条件 THEN 结论", "description": "描述",
     "linked_object_type_temp_ids": ["关联的类型临时ID"],
     "conditions": [{"field": "字段名", "op": ">|<|>=|<=|==|!=|in", "value": 阈值}]}
  ],
  "actions": [
    {"name_cn": "动作名", "execution_rule": "描述",
     "target_object_type_temp_id": "目标类型临时ID",
     "submission_criteria": [{"field": "字段名", "op": ">|<|>=|<=|==|!=|in", "value": 阈值}]}
  ]
}
...
1. **ObjectType**：
   - 识别文档中提到的所有"类型"（武器型号、战车型号、导弹实例、部队、设施等）
   - 每个 ObjectType 给出 property_schema（可量化字段），分配临时 ID（如 `ot-1`）

2. **ObjectInstance**——分两层：
   - **型号/单位级实例**（如"洲际弹道导弹""装甲师"这种全局就一个的东西）：直接作为 instance 输出
   - **批量个体实例**：如果文档说"导弹A共30枚，5枚损毁"，不要展开30行！用一个特殊字段 `bulk_count` 标记：
     `{"name_cn":"导弹A","bulk_count":30,"bulk_damaged":5,"bulk_pattern":"MissileA_{i:03d}","properties_template":{"序列号":"A-{i:04d}","状态":"{status}"}}`
     代码会自动展开这些批量实例

3. **Interface**：共享属性组才抽（两个以上类型都用到的字段组），只有一个类型用的不抽。

4. **Link**：关键 Link 必须输出——至少每个实例有一条 `instanceOf` 或 `equippedBy` 或 `deployedAt`。批量实例（有 bulk_count 的）只需给一条代表性 Link，代码会自动为全部个体生成。

5. **LogicRule + Action**：从文档的 IF-THEN 规则提取，conditions 的 field 必须在 property_schema 中存在，op 只允许 > < >= <= == != in。"""},
]
