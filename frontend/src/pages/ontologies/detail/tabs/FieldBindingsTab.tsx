...
export default function FieldBindingsTab({ ontologyId }: { ontologyId: string }) {
  ...
  function save() {
    if (!form.object_type_id || !form.property_name || !form.table_name || !form.column_name) {
      setError('请选择本体类型、属性，并填写表名和列名。')
      return
    }
    saveMut.mutate({
      ...form,
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        ...
        <button onClick={() => { setForm(emptyForm); setEditId(null); setError('') }} className="px-3 py-1.5 border rounded-lg text-xs flex items-center gap-1 text-gray-600 hover:bg-gray-50">
          <Plus size={13} /> 新建绑定
        </button>
      </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-[420px_1fr] gap-5">
        <div className="bg-white border rounded-xl p-5 space-y-4">
          <div className="flex items-center gap-2 text-sm font-medium text-gray-800">
            <Link2 size={15} /> {editId ? '编辑绑定' : '新建绑定'}
          </div>

          <div className="space-y-3">
            <div>
              <label className="block text-xs text-gray-500 mb-1">本体类型</label>
              <select value={form.object_type_id} onChange={e => update({ object_type_id: e.target.value, property_name: '' })} className="w-full border rounded-lg px-3 py-2 text-sm">
            ...
            <div>
              ...
              <div className="flex gap-2">
                <select value={form.data_source_id} onChange={e => {
                  ...
                  update({ data_source_id: e.target.value, table_name: source?.registered_table || '', column_name: '', primary_key_column: '' })
              ...
              <div>
                ...
                <div className="flex gap-2">
                  {form.data_source_id && sourceTables.length > 0 ? (
                    <select value={form.table_name} onChange={e => update({ table_name: e.target.value, column_name: '', primary_key_column: '' })} className="min-w-0 flex-1 border rounded-lg px-3 py-2 text-sm">
            ...
            <div className="grid grid-cols-2 gap-2">
              <div>
                <label className="block text-xs text-gray-500 mb-1">列名</label>
                <div className="flex gap-2">
            ...
            <div className="grid grid-cols-2 gap-2">
              <div>
                <label className="block text-xs text-gray-500 mb-1">值类型</label>
                <select value={form.value_type} onChange={e => update({ value_type: e.target.value })} className="w-full border rounded-lg px-3 py-2 text-sm">
            ...
            <div>
              <label className="block text-xs text-gray-500 mb-1">转换表达式</label>
              <textarea value={form.transform_expression} onChange={e => update({ transform_expression: e.target.value })} rows={3} placeholder="可选，例如 value.strip().upper()" className="w-full border rounded-lg px-3 py-2 text-sm font-mono" />
            </div>
          ...
          <button onClick={save} disabled={saveMut.isPending} className="w-full px-4 py-2 bg-black text-white rounded-lg text-sm flex items-center justify-center gap-2 disabled:opacity-40">
            <Save size={14} /> 保存绑定
          </button>
        </div>
